...
test('All 55 runtime resources are cached together, including sounds, icons, theme assets and modular controllers', async () => {
  const h = harness();
...
test('Cached navigation, modules, atlas, audio and icons work offline', async () => {
  ...
  const resources = [
    ...
    'src/ui/game-transition.js',
    'src/ui/leaderboard-ui.js',
    'src/ui/unranked-warning.js',
...
test('Waiting workers expose their build number to the page', async () => {
  ...
  assert.equal(result.build, '0.2.7.4b-dev11-hotfix1');
});
