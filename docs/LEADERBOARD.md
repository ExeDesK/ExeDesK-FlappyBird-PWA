...
## Résilience du suivi de détention (`020`)

Le suivi `leaderboard_record_state` est une donnée dérivée d'affichage. Depuis
`v0.2.8b-hotfix1`, son trigger est **best-effort** : une erreur de synchronisation
de la détention ne peut plus annuler la transition autoritaire d'une Verified Run
ni la mise à jour de `player_stats`. Le trigger UPDATE ne s'exécute que lorsqu'un
champ de record personnel (`best_score`, `best_run_id`, `best_score_at`) change
réellement, afin qu'une run ordinaire n'entre pas dans la section critique du
record mondial.
