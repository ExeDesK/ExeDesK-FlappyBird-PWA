...
export function renderQualityScale(dpr=1,{performance=false}={}){
  ...
  if(performance)return 2;
  return Math.min(3,Math.max(2,Math.ceil(ratio)));
}

export function rgb(values){return `rgb(${values[0]}, ${values[1]}, ${values[2]})`;}
...
export function compositeColor(base,fade=0,flash=0){
  const f=Math.max(0,Math.min(1,Number(fade)||0));
  const w=Math.max(0,Math.min(1,Number(flash)||0));
  const out=base.map(v=>Math.round((v*(1-f))*(1-w)+255*w));
  return rgb(out);
}
export function compositeLandColor(fade=0,flash=0){return compositeColor(LAND_FILL,fade,flash);}
export function compositeSkyColor(background='bg_day',fade=0,flash=0){
  return compositeColor(background==='bg_night'?SKY_NIGHT:SKY_DAY,fade,flash);
}

export function computeDisplaySize(stageWidth,stageHeight,{aspect='adapted'}={}){
  const w=Math.max(0,Number(stageWidth)||0);
  const h=Math.max(0,Number(stageHeight)||0);
  const scale=Math.max(0,Math.min(w/LOGICAL_WIDTH,h/LOGICAL_HEIGHT));
  const gameWidth=LOGICAL_WIDTH*scale;
  const gameHeight=LOGICAL_HEIGHT*scale;

  if(aspect==='adapted'&&scale>0){
    const extra=Math.max(0,h-gameHeight);
