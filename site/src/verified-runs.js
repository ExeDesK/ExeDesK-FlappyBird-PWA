...
export const RUN_RESULT_SCHEMA = 'flappy13-run-result-v1';
...
export function parseVerifiedRunSubmission(payload) {
  const value = requireObject(payload, 'Soumission de run');

  if (value.schema !== VERIFIED_RUN_SCHEMA) {
...
export function parseVerifiedRunResult(payload) {
  const value = requireObject(payload, 'Résultat de run');

  if (value.schema !== RUN_RESULT_SCHEMA) {
    throw new TypeError('Schéma de résultat de run non supporté.');
  }

  if (value.status !== 'verified' && value.status !== 'rejected') {
    throw new TypeError('Statut de résultat de run invalide.');
  }

  const resolvedAt = String(value.resolved_at || '');
  if (!resolvedAt || !Number.isFinite(Date.parse(resolvedAt))) {
    throw new TypeError('resolved_at invalide.');
  }
  ...
  const result = {
    ...
    physics_version: requirePhysicsVersion(value.physics_version),
    status: value.status,
    terminal_tick: value.terminal_tick === null
      ? null
      : requireRunTick(value.terminal_tick),
    score: value.score,
  ...
  if (result.status === 'verified') {
    if (
      result.terminal_tick === null ||
      !Number.isInteger(result.score) ||
      result.score < 0 ||
      result.score > INT32_MAX ||
      !COLLISIONS.has(result.collision) ||
      result.rejection_code !== null
    ) {
      throw new TypeError('Résultat vérifié incohérent.');
    }
  } else if (
    result.score !== null ||
    result.collision !== null ||
    typeof result.rejection_code !== 'string' ||
    !result.rejection_code
  ) {
    throw new TypeError('Résultat rejeté incohérent.');
  }
...
export function createCanonicalRunGame({
  ...
  const game = new Game({ seed: normalizedSeed, best: 0, headless });
...
export function simulateVerifiedRun({ seed, terminal_tick: terminalTick, taps } = {}) {
  ...
  const { game, warmupFrames } = createCanonicalRunGame({
    seed: normalizedSeed,
    headless: true,
  });
